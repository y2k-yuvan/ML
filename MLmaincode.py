...
def main():
    ...
    if soil_file and weather_file and yield_file:
        try:
            ...
            # Example features: 'Temperature', 'Rainfall', 'Soil Quality', 'Humidity'
            temperature = st.number_input("Temperature (°C)", min_value=0.0, max_value=50.0, value=25.0)
            rainfall = st.number_input("Rainfall (mm)", min_value=0.0, max_value=500.0, value=100.0)
            soil_quality = st.number_input("Soil Quality (1-10)", min_value=1, max_value=10, value=5)
            humidity = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0, value=60.0)

            # Make prediction if the button is clicked
            if st.button("Predict Crop Yield"):
                # Prepare input data for the model
                input_data = np.array([[temperature, rainfall, soil_quality, humidity]])
